...
5. Now, trace the `kvm_run_exit` event by adding `--trace kvm_run_exit,file=$HOME/qemu.out` to the QEMU command. This event lets us see when and why KVM returns control to QEMU. Make sure KVM is enabled. What reasons do you see? Use https://github.com/qemu/qemu/blob/master/linux-headers/linux/kvm.h to interpret them.
